...
/*
 解题思路:
 由于这题需要判断树的子结构,需要遍历二叉树的每个节点,然后每个节点与子结构的根节点比较,
 如果有相等的再判断是否相同的结构,如果没有,则返回false,如果相同则返回true.因此分为两部分:
 1.需要遍历二叉树节点的函数
 2.需要比较结构是否一致的函数.
  */
///写成两个函数的意义在于,如果在遍历二叉树节点的同时,进行比较,如果不满足就直接退出,无法再递归遍历,因此需要写两个函数递归嵌套!!!
bool doeshassubstruct(BinaryTreeNode *A,BinaryTreeNode *B)
{
    if(B == nullptr)
        return true;
    if(A == nullptr)
        return false;
    if(A->m_dbValue == B->m_dbValue)
    {
        return doeshassubstruct(A->m_pLeft,B->m_pLeft) && doeshassubstruct(A->m_pRight,B->m_pRight);
    } else
        return false;
}

bool hasbinarynode(BinaryTreeNode *A,BinaryTreeNode *B)
{
    bool result = false;
    ...
    if(A != nullptr&&B != nullptr)
        if(A->m_dbValue==B->m_dbValue)
        {
            result=doeshassubstruct(A->m_pLeft,B->m_pLeft)&&doeshassubstruct(A->m_pRight,B->m_pRight);
            if(!result)
                result=hasbinarynode(A->m_pLeft,B) ;
            if(!result)
                result=hasbinarynode(A->m_pRight,B);
        }
    return result;
}
...
{
    if(hasbinarynode(pRoot1, pRoot2) == expected)
        printf("%s passed.\n", testName);
    else
        printf("%s failed.\n", testName);
}
